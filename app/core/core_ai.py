...
VERSAO NumPy VETORIZADA — ~2100x mais rapida que v2.0 (Python puro)
MODELO: Double DQN + PER + Soft Target + Adam · 2000 episodios
"""
import math
import random
import json
...
# ==============================================================================
# SECAO 0 — CSV DE DADOS REAIS (embutido)
# ==============================================================================
RAW_CSV = """\
...
def sigmoid(v):
    return 1.0 / (1.0 + math.exp(-max(-500, min(500, v))))

# ==============================================================================
# SECAO 3 — REDE NEURAL MLP + ADAM + BACKPROP (NumPy vetorizado)
# ==============================================================================
class NeuralNetwork:
...
# ==============================================================================
# SECAO 4 — REPLAY BUFFER PRIORIZADO
# ==============================================================================
class PrioritizedReplayBuffer:
    ...
    def sample(self, k):
        ...
        beta = min(1.0, self.beta0 + self.frame*(1-self.beta0)/self.beta_f)
        self.frame += 1
        ws = [(n*probs[i])**(-beta) for i in idx]
        mw = max(ws)
        ws = [w/mw for w in ws]
    ...
    def update(self, idx, errs):
        for i, e in zip(idx, errs):
            self.pri[i] = abs(e) + 1e-6

    def __len__(self):
        return len(self.buf)

# ==============================================================================
# SECAO 5 — PARSER E NORMALIZACAO DO CSV REAL
# ==============================================================================
class RealDataEngine:
    ...
    def state_vector(self, m: dict) -> list:
        altitude_factor = min(1.0, m["altitude_h"] / 3000.0)
        xg_diff = m["xg_home"] - m["xg_away"]
        pos_diff = (m["away_pos"] - m["home_pos"]) / 18.0
    ...
    def compute_poisson_probs(self, xg_h: float, xg_a: float, max_goals: int = 8) -> dict:
        def pmf(k, lam):
    ...
    def h2h_stats(self) -> dict:
        ...
        if not h2h:
            return {}
        mur_w = sum(1 for m in h2h
                    if (m["home"] == "Mushuc Runa" and m["result"] == 0) or
                    (m["away"] == "Mushuc Runa" and m["result"] == 2))
        ldu_w = sum(1 for m in h2h
                    if (m["home"] == "LDU Quito" and m["result"] == 0) or
...
class DQNAgent:
    ...
    def train(self):
        ...
        qn, _ = self.online.forward(NS)
        qt, _ = self.target.forward(NS)
        qc, acts = self.online.forward(S)
        ba = np.argmax(qn, axis=1)
...
class Evaluator:
    ...
    def pdo_signal(pdo):
        if pdo > 1100:
            return "OVERPERFORMING -> regressao esperada"
        if pdo < 900:
            return "UNDERPERFORMING -> recuperacao esperada"
        return "NEUTRO (~1000)"

# ==============================================================================
# SECAO 10 — TRAINER (2000 EPISODIOS)
# ==============================================================================
class Trainer:
    ...
    def run(self):
        ...
        print(f"  H2H historico (MURxLDU)   : {h2h.get('total_h2h', 0)} partidas")
        print(f"    MUR venceu: {h2h.get('mur_wins', 0)}  | LDU venceu: {h2h.get('ldu_wins', 0)}  | Empates: {h2h.get('draws', 0)}")
        print(f"    Media de gols H2H        : {h2h.get('avg_goals_h2h', 0)}")
        print(f"  Episodios de treino       : {self.episodes}")
        ...
        for ep in range(1, self.episodes + 1):
            ...
            while not done:
                ...
                if result >= 0:
                    self.all_acts.append(result)
                else:
                    self.all_acts.append(0)
                ns, r, done, info = self.env.step(a, mp)
                self.agent.push(s, a, r, ns, done)
                if self.agent.steps % self.train_freq == 0:
                    self.agent.train()
                ep_r += r
                s = ns
            ...
            if ep % self.log_freq == 0 or ep == 1:
                self._log(ep, t0)
        print("\n" + "=" * 68)
        print("  TREINAMENTO CONCLUIDO - ANALISANDO PARTIDA ALVO")
        print("=" * 68)
        self._final(t0)

    def _log(self, ep, t0):
        w = min(200, ep)
        rr = self.ep_rewards[-w:]
        rb = self.ep_banks[-w:]
        ar = sum(rr) / len(rr)
        ab = sum(rb) / len(rb)
        pp = self.all_preds[-w*200:]
        aa = self.all_acts[-w*200:]
        ...
        print(f"[Ep {ep:>4}/{self.episodes}] "
              ...
              f"eps:{self.agent.eps:.3f} Loss:{al:.4f} "
...
class MatchAnalyzer:
    ...
    def analyze_target(self, bankroll=1000.0) -> dict:
        ...
        confidence = "ALTA" if sq[0] - sq[1] > 0.5 else ("MEDIA" if sq[0] - sq[1] > 0.2 else "BAIXA")
        alt_penalty = 0.0
        ...
        return {
            ...
            "formations": "MUR 4-4-2 (Velez) - LDU 5-3-2 (T. Nunes)",
            "injuries": "MUR: Mina (duvida) - LDU: sem baixas confirmadas",
            ...
            "corners_over_9.5": "SIM (7/7 ultimos H2H)",
            "under_2.5_gols": "LDU: 9 ultimas partidas UNDER",
        }
...
# ==============================================================================
# SECAO 12 — ENTRY POINT PRINCIPAL (COMENTADO PARA API)
# ==============================================================================
def main():
    random.seed(42)
    print("\n" + "=" * 68)
    print("  DRL FOOTBALL BETTING SYSTEM v2.0")
    print("  MUSHUC RUNA SC vs L.D.U. QUITO")
    print("  Liga Pro Ecuabet 2026 - J13")
    print("=" * 68 + "\n")
    cfg = {
        "episodes": 2000,
        "bankroll": 1000.0,
    ...
    print("\n" + "=" * 68)
    print("  ANALISE FINAL - MUSHUC RUNA SC vs L.D.U. QUITO - 12/05/2026")
    print("=" * 68)
    ...
    print("\n" + "=" * 68)
    print("  RESUMO EXECUTIVO - RECOMENDACAO DE APOSTA")
    print("=" * 68)
    print(f"  Partida  : {a['match']}")
    print(f"  Data     : {a['date']}")
    print(f"  Acao DRL : > {best} <   (confianca: {a['confianca']})")
...
# COMENTADO PARA NAO RODAR TREINAMENTO QUANDO IMPORTADO PELA API
